module Page.TeacherDashboard exposing (Model, Msg, loadMistakeTypes, loadStudentPerformance, loadSubjectPerformance, model, update, view)
...
subjectPerformanceTransformer dict =
    Debug.log "subjectPerformance list"
        (List.map
            (\subjectName ->
                ( Subject (Debug.log "subjectPerformance subject" subjectName)
            ...
            (Dict.keys (Debug.log "subjectPerformance dict" dict))
        )
...
update session msg model =
    case msg of
        LoadStudentPerformance (Ok resp) ->
            { model | lineChartModel = Debug.log "lineChart" resp.result } ! []
        ...
        LoadSubjectPerformance (Ok resp) ->
            { model | barChartModel = Debug.log "barChart" resp.result } ! []
        ...
        LoadMistakeTypes (Ok resp) ->
            { model | pieChartModel = Debug.log "pieChart" resp.result } ! []
